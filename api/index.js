...
async function fetchAllNews() {
    const [globalNews, techNews, aiNews] = await Promise.all([
        fetchSerpNews('world news viral today breaking'),
        fetchSerpNews('technology news viral latest'),
        fetchSerpNews('AI artificial intelligence OpenClaw news')
    ]);
    
    return {
        global: processNewsResults(globalNews, ['BREAKING', 'WORLD', 'POLITICS']),
        tech: processNewsResults(techNews, ['TECH', 'ECONOMY']),
        ai: processNewsResults(aiNews, ['AI', 'SYNTHETIC MEDIA', 'LIVE STREAM'])
    };
}
...
function processNewsResults(results, categories) {
    if (!results || results.length === 0) return [];
    
    return results.slice(0, 4).map((item, index) => {
